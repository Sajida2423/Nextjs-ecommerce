...
export default async function Detailpage(props: { params: { productid: string } }) {
  const prodid = Number(props.params.productid);
  const singleproduct = await Productservice.getproductbyid(prodid);

  if (!singleproduct) return <p className="text-center mt-5">Product not found</p>;
  ...
  return (
    <div className="container mt-3 d-flex flex-column min-vh-100">
      <div className="card p-3 shadow" style={{ backgroundColor: "#f8e6f0" }}>
        <div className="row">
          <div className="col-md-5 text-center">
            <img src={singleproduct.images[0]} alt={singleproduct.title} className="img-fluid" style={{ height: "180px", objectFit: "contain" }} />
          </div>
          <div className="col-md-7">
            <h3 className="mb-3">{singleproduct.title}</h3>
            <h4 className="text-success mb-3">₹{singleproduct.price}</h4>
            <p className="mb-2"><strong>Category:</strong> {singleproduct.category}</p>
            <p className="text-muted mb-4">{singleproduct.description}</p>
            <div className="d-flex align-items-center gap-3">
              <AddToCartButton product={{ id: singleproduct.id, title: singleproduct.title, price: singleproduct.price, image: singleproduct.images[0], quantity: 1 }} />
            </div>
          </div>
        </div>
      </div>
